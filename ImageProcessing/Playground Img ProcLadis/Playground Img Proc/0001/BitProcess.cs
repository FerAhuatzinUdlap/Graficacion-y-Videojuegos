...
namespace _0001
{
    public class BitProcess
    {
        ...
        private static void BrightnessPixel(byte[] bits, int div, int i, int idx, int x)
        {
            int newRed;
            int newBlue;
            int newGreen;

            newRed = (int)bits[(i * div) + idx + r] + x;
            newGreen = (int)bits[(i * div) + idx + g]+ x;
            newBlue = (int)bits[(i * div) + idx + b]+ x;
        ...
        public static byte[] Brightness(byte[] bits, int x)
        {
            int div = 16;
            Parallel.For(0, bits.Length / div, i => // unrolling 
            {
                BrightnessPixel(bits, div, i, 0, x);
                BrightnessPixel(bits, div, i, 4, x);
                BrightnessPixel(bits, div, i, 8, x);
                BrightnessPixel(bits, div, i, 12, x);
            });

            return bits;
        }

        private static void ContrastPixel(byte[] bits, int div, int i, int idx, double alpha)
        {
            int newRed;
            int newBlue;
            int newGreen;

            newRed = (int)(alpha * ((int)bits[(i * div) + idx + r] - 128) + 128);
            newGreen = (int)(alpha * ((int)bits[(i * div) + idx + g] - 128) + 128);
            newBlue = (int)(alpha * ((int)bits[(i * div) + idx + b] - 128) + 128);
        ...
        public static byte[] Contrast(byte[] bits, double alpha)
        {
            int div = 16;
            Parallel.For(0, bits.Length / div, i => // unrolling 
            {
                ContrastPixel(bits, div, i, 0, alpha);
                ContrastPixel(bits, div, i, 4, alpha);
                ContrastPixel(bits, div, i, 8, alpha);
                ContrastPixel(bits, div, i, 12, alpha);
            });

            return bits;
        }

        public static float[,] getHistogram(byte[] bits)
        {
            //3 channels X 256 levels of intensity per channel
            float[,] histogram = new float[3, 256];
            int div = 16;

            Parallel.For(0, bits.Length / div, i => // unrolling 
            {
                addToChannels(bits, div, i, 0, ref histogram);
                addToChannels(bits, div, i, 4, ref histogram);
                addToChannels(bits, div, i, 8, ref histogram);
                addToChannels(bits, div, i, 12, ref histogram);
            });
            normalizeHistogram(ref histogram, bits.Length);
            return histogram;
        }

        public static void addToChannels(byte[] bits, int div, int i, int idx, ref float[,] histogram)
        {
            //in their respective channel add one to the histogram to the level intensity that corresponds
            histogram[0, bits[(i * div) + idx + r]]++;
            histogram[1, bits[(i * div) + idx + g]]++;
            histogram[2, bits[(i * div) + idx + b]]++;
        }

        public static void normalizeHistogram(ref float[,] histogram, int length)
        {
            //get all values normalized meaning that the addition of all of them should be 100
            float numberOfPixels = 0;
            for (int i = 0; i < histogram.GetLength(0); i++)
            {
                for (int j = 0; j < histogram.GetLength(1); j++)
                {
                    numberOfPixels += histogram[i, j];
            ...
            for (int i = 0; i < histogram.GetLength(0); i++)
            {
                for (int j = 0; j < histogram.GetLength(1); j++)
                {
                    histogram[i, j] = ((histogram[i, j] / numberOfPixels) * 100);
                }
            }
